...
public class MainController
{
	...
	@RequestMapping("listByFollow")
	public Object listByFollow(@RequestParam(defaultValue="1") int pageNo, int uno) throws Exception {
		List<String> tagList = MainDao.selectUserTag(uno);
		int pageSize = 20;
		HashMap<String,Object> paramMap = new HashMap<>();
		paramMap.put("startIndex", (pageNo - 1) * pageSize);
		paramMap.put("length", pageSize);
		...
		System.out.println("pageNo : " + pageNo);
		List<Wish> wishs = MainDao.selectListByFollow(paramMap);
	...
	public AjaxResult addComment(Comment comment,HttpServletRequest req) throws Exception {
		...
		comment.setCoNo(MainDao.selectCoNo());
